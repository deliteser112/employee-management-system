<template>
    <div>
        <q-dialog 
            ...
            transition-hide="rotate" @show="fetchEmployee">
            <q-card class="dialog-wrapper">
                ...
                <q-card-actions align="right">
                    <q-btn flat label="Cancel" color="negative" @click="closeEditEmployeeForm" />
...
const fetchEmployee = async () => {
    const employee: Employee = await store.dispatch('findEmployee', props.id)
...
const openDialog = computed(() => store.state.showEditEmployeeForm)
const closeEditEmployeeForm = () => {
    isSubmitting.value = false
    store.dispatch('closeEditEmployeeForm')
}
...
const onSubmit = async () => {
    ...
    await store.dispatch('editEmployee', emp)
    isSubmitting.value = false
    name.value = ""
    department.value = ""
    age.value = 0
